...
def merge_crf_into_schedule_grid(schedule_grid_df, crf_raw_df):
    ...
    try:
        ...
        for form in unique_forms:
            ...
            if event_codes_str and event_codes_str != 'nan':
                # Split by comma, semicolon, or space
                event_codes_list = re.split(r'[,;\s]+', event_codes_str)
                event_codes_list = [code.strip().upper() for code in event_codes_list if code.strip()]

            print(f"[MERGE] Form '{form['form_label']}' has event codes: {event_codes_list}")

            # Check each column and mark X if event code matches
            for col_idx, col_name in enumerate(schedule_columns[2:], start=2):
                # Get the Event Name from schedule grid (row 1)
                if len(schedule_grid_df) > 1:
                    event_name = str(schedule_grid_df.iloc[1][col_name]).strip().upper()

                    # Check if any event code matches this column's event name
                    if any(code == event_name or code in event_name or event_name in code
...
# Download section - Combined Excel File (Primary Output)
if st.session_state.combined_excel is not None:
    st.markdown("---")
    st.subheader("📥 Combined Excel File (Recommended)")
    col1, col2 = st.columns([3, 1])
    with col1:
        st.success("✅ Combined Excel file ready with both Schedule Grid and CRF data")
        st.info("📊 This file contains both 'Schedule Grid' and 'Study Specific Forms' sheets")
    with col2:
        st.download_button(
