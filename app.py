...
# --- Existing Code Functions (Copy and Paste from your notebook) ---
...
# Function to process the Protocol REF PDF and extract table data
def process_protocol_pdf(pdf_path: str) -> pd.DataFrame:
    """
    ...
    if start_page is not None and end_page is not None:
        ...
        try:
            tables = camelot.read_pdf(extracted_pdf_path, pages='all', process_background=False, split_text=True, line_scale=47, flag_size=False)

            if tables:
                all_t = pd.DataFrame()
                rc = 0.00
                for table in tables:
                    all_t = pd.concat((all_t, table.df), ignore_index=True)
                    rc += float(dict(table.parsing_report)['accuracy'])

                st.write(f"Camelot average accuracy: {rc / len(tables):.2f}")

                pr_df = all_t.drop_duplicates().reset_index(drop=True)
                ...
                return pr_df.dropna(axis=1, how='all') # Drop columns that are entirely NaN after processing

            else:
                st.warning("No tables found in the extracted Protocol REF section using Camelot.")
                return pd.DataFrame()

        except Exception as e:
            st.error(f"Error during Camelot table extraction: {e}")
            return pd.DataFrame()

    elif start_page is None:
        st.error("Could not find the 'Schedule of Activities' section heading in Protocol REF.")
        return pd.DataFrame()
    else:
        st.error("Could not find the 'Introduction' section heading in Protocol REF.")
        return pd.DataFrame()
...
if st.button("Process Documents"):
    if uploaded_crf_file is not None and uploaded_protocol_file is not None:
        ...
        if crf_chunks:
            ...
            if not crf_extraction_df.empty:
                ...
                st.download_button(
                    ...
                    data=crf_excel_data,
                    file_name='extracted_crf_data.xlsx',
        ...
        st.subheader("Protocol REF Processing Results (Schedule of Activities)")
        # Process Protocol REF
        protocol_df = process_protocol_pdf(protocol_filename)

        if not protocol_df.empty:
            st.write("Extracted Schedule of Activities Table:")
            ...
            st.download_button(
                label="Download Schedule of Activities as Excel",
                data=protocol_excel_data,
                file_name='schedule_of_activities.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            )
        else:
            st.warning("No data extracted from the Protocol REF (Schedule of Activities).")
...
st.markdown(
    """
    To run this Streamlit app in Google Colab:
    1. Execute this cell to create the `app.py` file.
    2. In a new cell, run: `!streamlit run app.py & npx localtunnel --port 8501`
    3. Click the public URL provided by localtunnel.
    """
)
